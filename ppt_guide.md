...
[Bottom bar]  Powered by ACLED Data · XGBoost · SHAP Explainability
...
📥 INPUT                    🧠 MODEL                    📤 OUTPUT
────────                    ───────                     ────────
ACLED Weekly Data           XGBoost ML                  83 Province
89,297 Records              33 Features                 Risk Scores
4 Countries                 PR-AUC: 0.90+               + SHAP Explanations
9+ Years History            F1: 0.84+                   + Interactive Map
```
...
**Layout:** Left = how XGBoost works, Right = hyperparameters table

### Content
```
[Headline]  Model: XGBoost Gradient Boosting
...
[Left — simplified explainer with icons]
  ...
  Tree 3: Correct Tree 2's mistakes
      ...
      ... × 400 trees
      ↓
  Final prediction = sum of all 400 trees × 0.05
...
[Right — parameter table]
  ...
  Operating threshold = 0.20 (tuned via F1 sweep)
```
...
[4 big KPI cards in a row, teal/white on dark]
  F1-Score    PR-AUC    ROC-AUC    Operating
  0.84+       0.90+     0.82       Threshold: 0.20

[Baselines comparison table]
  Strategy               F1     vs. Our Model
  ─────────────────────────────────────────
  Always predict "1"     0.81   − 0.03
  Majority class         0.81   − 0.03
  Persistence (last)     0.83   − 0.01
  Event heuristic        0.80   − 0.04
  ─────────────────────────────────────────
  ✅ XGBoost (ours)      0.84   WINNER

[Bottom note]
  Train: 2017–2023 (19,801 rows) · Test: 2025–2026 (5,204 rows)
...
[Right sidebar — Top 5 insights]
  #1  fatalities_w30d (0.54)
      Sustained deaths = strongest signal
  #2  velocity_events_w30d (0.33)
      Acceleration matters more than raw counts
  #9  spillover_w14d (0.14)
      Violence spreads to neighbours
  #8  month (0.14)
...
### Content
```
[Headline]  Case Study: Why Is Magway "Critical" and Sikkim "Low"?

[Left — HIGH RISK case]
  Mandalay, Myanmar — Predicted: 99.8% 🔴 Critical
...
[Left — Summary Card in teal border box]
  ...
  ✅  XGBoost model: F1=0.84, PR-AUC=0.90
...
[Bottom — Thank you line]
  Data source: ACLED (acleddata.com) · Model: XGBoost · Explainability: SHAP
```
...
- **Big numbers in teal** for KPIs (F1: 0.84, PR-AUC: 0.90, 83 provinces)
